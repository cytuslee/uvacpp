...
void MapXYToLatLon(double x, double y, double lambda0,
  ...
  /* Calculate latitude */
  p.X = phif + x2frac * x2poly * (x * x) + x4frac * x4poly * pow(x, 4.0) +
        x6frac * x6poly * pow(x, 6.0) + x8frac * x8poly * pow(x, 8.0);

  /* Calculate longitude */
  p.Y = lambda0 + x1frac * x + x3frac * x3poly * pow(x, 3.0) +
 ...
 *
 */
pair<double, double> LatLonToUTMXY(double lat, double lon) {
  ...
  if (xy.Y < 0.0) xy.Y = xy.Y + 10000000.0;

  return make_pair(xy.X, xy.Y);
}
/*
 * UTMXYToLatLon
 *
 ...
 *
 */
pair<double, double> UTMXYToLatLon(double x, double y, double zone,
                                   bool southhemi) {
  double cmeridian;
...
int main() {
  ...
  pair<double, double> WGS = make_pair(112.52116394043, 23.1911296844482);
  pair<double, double> x = LatLonToUTMXY(WGS.Y, WGS.X);
  cout << setprecision(15) << x.X << "," << x.Y << endl;
  pair<double, double> UTM = make_pair(655683.25, 2565535.25);
  pair<double, double> y = UTMXYToLatLon(UTM.X, UTM.Y, 49, false);
  cout << setprecision(15) << y.X << "," << y.Y << endl;
  return 0;
}